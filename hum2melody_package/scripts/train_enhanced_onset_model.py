"""
Train Enhanced Onset/Offset Model

Trains an enhanced onset/offset detection model with:
- Musical components (HarmonicCNN, multi-scale encoder)
- Rich input features (onset strength + musical context)
...
Key features:
- Uses EnhancedMelodyDataset (112 channels: 88 CQT + 5 onset + 19 musical)
- HarmonicCNN with dilated convolutions
- Multi-scale temporal encoder
- Cross-attention mechanism
- Improved onset/offset loss with pos/neg balancing

Usage:
    python scripts/train_enhanced_onset_model.py \\
        ...
        --lr 1e-4 \\
        --epochs 50 \\
        --checkpoint-dir checkpoints_enhanced_onset
"""
...
def parse_args():
    ...
    parser.add_argument("--input-channels", type=int, default=112, help="Input channels (88 CQT + 5 onset + 19 musical)")
    parser.add_argument("--hidden-size", type=int, default=256, help="LSTM hidden size (increased for enhanced model)")
    parser.add_argument("--dropout", type=float, default=0.3, help="Dropout rate")
    parser.add_argument("--use-multi-scale", action='store_true', default=True, help="Use multi-scale encoder")
    parser.add_argument("--use-cross-attention", action='store_true', default=True, help="Use cross-attention")

    # Training
    parser.add_argument("--batch-size", type=int, default=16)  # Reduced for larger model
    ...
    # Checkpoints
    parser.add_argument("--checkpoint-dir", type=str, default="checkpoints_enhanced_onset")
...
def save_checkpoint_with_metadata(
    ...
    checkpoint = {
        ...
        'architecture': {
            ...
            'n_bins': model.n_bins,
            'input_channels': model.input_channels,  # Enhanced model uses 112 channels
...
def main():
    ...
    # This provides 112 channels: 88 CQT + 5 onset features + 19 musical context
    ...
    dataset = EnhancedMelodyDataset(
        ...
        augment=True,
        spec_augment=True,
        use_onset_features=True,  # Add onset strength features (5 channels)
        use_musical_context=True,  # Add musical context features (19 channels)
        use_pretrained=False  # Skip pretrained features (too large)
    )

    print(f"✅ Enhanced dataset created:")
    print(f"   CQT bins: {dataset.n_bins}")
    print(f"   Onset features: 5 channels")
    print(f"   Musical context: 19 channels")
    print(f"   Total input channels: {args.input_channels}")
